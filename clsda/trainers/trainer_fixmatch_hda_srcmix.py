...
@VALIDATOR.register_module(name='fixmatchhdasrcmix')
class ValidatorFixMatchHDASrcMix(BaseValidator):
    def __init__(self, basic_parameters):
        super(ValidatorFixMatchHDASrcMix, self).__init__(**basic_parameters)
    ...
    def eval_iter(self, val_batch_data):
        ...
        with torch.no_grad():
            feat, pred_unlabeled, target_logits = self.model_dict['base_model'](val_img)
        return {'img': val_img,
                'gt': val_label,
                'img_metas': val_metas,
                'feat': feat,
...
@TRAINER.register_module('fixmatchhdasrcmix')
class TrainerFixMatchHDASrcMix(BaseTrainer):
    def __init__(self, basic_parameters,
                 # new parameters
                 ...
                 lambda_fixmatch=1.0,
                 #
                 lambda_adv=1.0, lambda_heuristic=1.0,
                 src_mixup=False, src_ce_type='weak',
                 tgt_sample_type='low',
                 toalign=False, loss_type=0, target_domain_label=1,

                 ):
        super(TrainerFixMatchHDASrcMix, self).__init__(**basic_parameters)
    ...
    def train_iter(self, *args):
        ...
        tmp_res = base_model(all_labeled_img, all_tgt_img, src_mixup=self.src_mixup, threshold=tmp_threshold,
                             ...
                             src_gt=src_label_strong, toalign=self.toalign, src_labels=src_label_strong,
                             ad_net=ad_net, train_iter=self.iteration)
        ...
        if self.toalign:
            online_src_toalign_logits = online_output['src_toalign_logits']
            outputs = torch.cat((online_src_toalign_logits, outputs_target), dim=0)
        else:
            outputs = torch.cat((outputs_source, outputs_target), dim=0)
        focals = torch.cat((focal_source, focal_target), dim=0)
        softmax_out = F.softmax(outputs, dim=1)
        #
        transfer_loss, mean_entropy, heuristic = HDALoss([softmax_out, focals], ad_net, calc_coeff(self.iteration),
                                                         iteration=self.iteration, split_num=focal_source.shape[0],
                                                        domain_label=src_domain_label,
                                                         target_domain_label=self.target_domain_label)
        # 监督损失
        ...
        loss = loss_supervised + self.lambda_adv * transfer_loss + heuristic * self.lambda_heuristic
