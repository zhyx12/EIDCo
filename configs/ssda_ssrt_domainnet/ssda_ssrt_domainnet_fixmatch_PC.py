_base_ = [
    '../_base_/cls_datasets/ssrt_ssda_domainnet_3shot/ssda_domainnet_3shot_4gpu_PC.py',
    '../_base_/cls_models/ssda_domainnet_toalign_srcmix/ssrt_vit_b_msda_domainnet_temp_0.03_lr_1e2.py'
]

log_interval = 100
val_interval = 1000

control = dict(
    log_interval=log_interval,
    max_iters=50000,
    val_interval=val_interval,
    cudnn_deterministic=False,
    save_interval=500,
    max_save_num=1,
    # seed=2,
)

train = dict(
    lambda_info_nce=0.0,
    lambda_fixmatch=1.0,
    lambda_kld=0.1,
    lambda_temp=0.3,
    prob_threshold=0.85,
    src_mixup=False,
    beta_param=0.2,
)

test = dict(
custom_hooks=[
    dict(type='ClsAccuracy', dataset_index=0, pred_key='pred'),
    dict(type='ClsAccuracy', dataset_index=0, pred_key='target_pred'),
    dict(type='ClsAccuracy', dataset_index=1, pred_key='pred'),
    dict(type='ClsAccuracy', dataset_index=1, pred_key='target_pred'),
    dict(type='ClsBestAccuracyByVal', patience=100, priority="LOWEST")
    ]
)
